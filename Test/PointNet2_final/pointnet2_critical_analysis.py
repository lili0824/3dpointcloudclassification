...
from pointnet2_inference import load_pointnet2_model
...
def load_ensemble_models(
    ...
    *,
    ...
    for fold in folds:
        ...
        model = load_pointnet2_model(checkpoint, num_classes=num_classes, use_normals=False)
...
def compute_ensemble_saliency(models, pts: np.ndarray, device: torch.device) -> np.ndarray:
    ...
    for model in models:
        ...
        logits, _ = model(pts_tensor.transpose(2, 1))
    ...
    *,
    ...
    for test_name in sorted(test_names):
        ...
        if source_npy_dir is None:
            print(f'⏭️  {test_name}: No NPY files found in dataset root or batch output')
            continue
